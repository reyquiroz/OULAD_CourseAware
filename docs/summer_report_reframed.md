...
1. A leakage-safe, enrollment-centric heterogeneous graph schema for OULAD with explicit temporal guards on both assessment due dates and submission dates.
2. An inductive training protocol that excludes held-out student activity from message-passing during LCPO evaluation.
3. A matched LightGBM baseline with identical features, split definitions, and validation-set threshold selection.
4. A course-level diagnostic framework correlating per-fold prediction performance with six course-design factors.
5. Multi-seed variance estimates (5 model seeds per fold) for all 22 LCPO fold results.
...
**Enrollment-centric supervision.** The prediction unit is the enrollment triple (id_student, code_module, code_presentation). One student may contribute multiple enrollments with different outcomes; these are treated as independent prediction instances. Aggregating at the student level or deduplicating by student identity would conflate presentations with different outcomes and is not performed anywhere in the pipeline.

**Normalisation.** Feature statistics (mean and variance for continuous features; category counts for one-hot features) are estimated exclusively from the training split of each fold and then applied to the full graph. This prevents any information from validation or test enrollments from influencing the representation.

**Split protocols.** Two protocols are used. Random-student splitting assigns complete student histories to train/validation/test partitions at the student level in a 70/10/20 ratio across five random seeds. Leave-course-presentation-out (LCPO) evaluation holds out one complete course presentation at a time; at week 8 this produces 22 folds covering 7 modules (AAA, BBB, CCC, DDD, EEE, FFF, GGG).

**Multi-seed variance estimation.** For LCPO, five independent model seeds are run per fold. The reported fold result is the mean ± standard deviation across seeds. This separates estimation variance (model initialisation) from generalisation variance (fold difficulty), and flags folds where high seed std indicates unstable learning rather than genuinely superior LightGBM performance.

**Matched comparison.** The LightGBM baseline receives identical features, identical split assignments, and applies the same validation-set threshold selection procedure used for the GNN. No hyperparameter advantage is given to either model.
...
is not performed because student populations, outcome definitions, course structures,
and learning environments differ across institutions and platforms.

### X.1 OULAD (Primary dataset)
See Section 5 for full results.

### X.2 KDD Cup 2015 / XuetangX (Planned)
*Results pending. See `docs/ku_leuven_feasibility.md` for feasibility assessment.*

| Metric | GraphSAGE | LightGBM |
|--------|-----------|----------|
| AUROC  | —         | —        |
| AUPRC  | —         | —        |
| F1     | —         | —        |

*Cross-dataset comparison will be reported once XuetangX results are available,
with explicit acknowledgement of dataset-specific differences in outcome definition
and graph schema coverage.*

### X.3 Future datasets
Canvas (institutional, UNT) and MOOCCubeX are reserved for dissertation-scale
or broader future work.
...
3. **Non-negligible seed variance in some folds.** Performance variance across model seeds is non-negligible in several LCPO folds. In particular, folds with small held-out cohorts or extreme class imbalance show high within-fold standard deviation. Conclusions drawn from individual fold results should account for this variance.

4. **Modest LCPO margin.** The GNN LCPO advantage of 0.020 AUROC is consistent but modest. This margin may not replicate on other datasets, particularly if those datasets lack the dense relational structure of OULAD (multiple presentations per module, overlapping student cohorts, rich VLE logs).

5. **LCPO independence assumption.** The 22 LCPO folds are not fully independent. Multiple presentations within the same module (e.g., BBB has four presentations) share students, course design, and instructor-level decisions. The effective number of independent evaluation units is closer to 7 (modules) than 22 (folds). Fold-level results should be interpreted with this dependence in mind.
